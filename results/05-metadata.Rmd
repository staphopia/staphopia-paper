...
staphopia::write_plot(p, paste0(getwd(), '/images/figure-x-published-per-year'))
```
...
#### Countries
```{r}
country_data <- as.data.frame(table(metrics[(metrics$country != "unknown/missing" ) & (metrics$country != ""),]$country))
...
#### Isolation Sources
```{r}
df <- as.data.frame(table(substr(tolower(metrics[metrics$isolation_source != "",]$isolation_source), 1, 50)))
df[order(-df$Freq),]
```

#### Number of Isolation Sources
```{r}
nrow(as.data.frame(table(tolower(metrics[metrics$isolation_source != "",]$isolation_source))))
```
